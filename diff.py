"""
Diff functions for comparing LLM1 vs LLM2 extraction results (v9).

Helpers for source-line-based episode matching and field comparison.
Entry points (one per step):
  diff_step1(logs_dir)  → step1_diff_report.json   (起止时间, 供职单位, 职务)
  diff_step2(logs_dir)  → step2_diff_report.json   (组织标签, 标志位, 任职地, 中央/地方)
  diff_step3(logs_dir)  → step3_diff_report.json   (rank)
  diff_step4(logs_dir)  → step4_diff_report.json   (bio + labels + 落马)
"""
...
def group_by_source_line(episodes: list[dict]) -> dict[int, list[dict]]:
    ...
    for i, ep in enumerate(episodes):
        sl = ep.get("source_line", i + 1)
        groups[sl].append(ep)
    return dict(groups)


def diff_episode_groups_step1(
    ds_group: list[dict], vf_group: list[dict],
    source_line: int,
) -> list[dict]:
    """Compare two groups of step1 episodes from the same source_line.

    step1 fields only: 起始时间, 终止时间, 供职单位, 职务.
    """
    diffs = []
    ...
    if len(ds_group) == len(vf_group):
        ...
        for ep_ds, ep_vf in zip(ds_sorted, vf_sorted):
            diffs += _diff_step1_pair(ep_ds, ep_vf, source_line)
    else:
        ...
        for i in range(min(len(ds_sorted), len(vf_sorted))):
            diffs += _diff_step1_pair(ds_sorted[i], vf_sorted[i], source_line)

    return diffs


def _diff_step1_pair(ep_ds: dict, ep_vf: dict, source_line: int) -> list[dict]:
    """Compare two step1 episodes (time, unit, position only)."""
    diffs = []
...
def diff_all_step1(eps_ds: list[dict], eps_vf: list[dict]) -> list[dict]:
    """Diff step1 episodes using source_line grouping."""
    ...
    for line_num in all_lines:
        ...
        if not ds_g and vf_g:
            ...
            diffs += diff_episode_groups_step1(ds_g, vf_g, line_num)

    return diffs


# ── Bio / label / corruption diffs (used by step4) ───────────────────────────
...
def diff_corruption(ds_s: dict, vf_s: dict) -> list[dict]:
    diffs = []
    ds_luoma = ds_s.get("是否落马", "")
    vf_luoma = vf_s.get("是否落马", "")
...
# ── Diff: Step 1 (basic episode structure) ──────────────────────────────────

def diff_step1(logs_dir: Path) -> Path:
    """Diff LLM1 vs LLM2 step1 (time/unit/pos). Saves step1_diff_report.json."""
    logger.info("=== Step1 Diff: LLM1 vs LLM2 (起止时间/供职单位/职务) ===")
...
# ── Diff: Step 2 (classification on merged_step1 episodes) ──────────────────

def diff_step2(logs_dir: Path) -> Path:
    """Diff step2 classifications (org tag, position tag, location).

    Compares LLM1 vs LLM2 classifications, indexed by episode_idx that was
    fed in from merged_episodes_step1.json.
    """
    logger.info("=== Step2 Diff: LLM1 vs LLM2 classifications ===")

    llm1_cache = load_json_cache(logs_dir / "llm1_step2_classify.json")
    llm2_cache = load_json_cache(logs_dir / "llm2_step2_classify.json")
    ...
    for name, ds_s2 in llm1_cache.items():
        ...
        ds_cls = {c.get("episode_idx", 0): c for c in ds_s2.get("classifications", [])}
        vf_cls = {c.get("episode_idx", 0): c for c in vf_s2.get("classifications", [])}

        ep_data = merged_step1.get(name, {})
        episodes = ep_data.get("episodes", [])
        ep_map = {ep.get("经历序号", i + 1): ep for i, ep in enumerate(episodes)}

        person_diffs: list[dict] = []
        all_indices = sorted(set(ds_cls) | set(vf_cls))
        ...
        for idx in all_indices:
            ...
            for field in STEP2_EPISODE_FIELDS:
                v_ds = str(ds_c.get(field, ""))
                v_vf = str(vf_c.get(field, ""))
                if v_ds == v_vf:
                    continue
                level = "HIGH" if field in ("组织标签", "标志位") else "MEDIUM"
                person_diffs.append({
                    "scope": "classify",
                    "field": field,
                    "episode_idx": idx,
                    "source_line": ep.get("source_line", 0),
                    "供职单位": ep.get("供职单位", ""),
                    "职务": ep.get("职务", ""),
                    "起始时间": ep.get("起始时间", ""),
                    "llm1_value": v_ds,
    ...
    total = len(all_diffs)
    passes = sum(1 for d in all_diffs if d["verdict"] == "PASS")
    logger.info(f"Step2 Diff 完成: PASS={passes} / {total}")
...
def diff_step3(logs_dir: Path) -> Path:
    """Diff LLM1 vs LLM2 step3 rank results. Saves step3_diff_report.json."""
    logger.info("=== Step3 Diff: LLM1 vs LLM2 rank ===")
    ...
    for name, ds_s3 in llm1_cache.items():
        ...
        ds_ranks = {r.get("episode_idx", 0): r.get("final_rank", "") for r in ds_s3.get("ranks", [])}
        vf_ranks = {r.get("episode_idx", 0): r.get("final_rank", "") for r in vf_s3.get("ranks", [])}
    ...
    logger.info(f"Step3 Diff 完成: {n_with_diffs} 人有 rank 差异 / {total}")

    return output_path


# ── Diff: Step 4 (labels + bio + corruption) ────────────────────────────────

def diff_step4(logs_dir: Path) -> Path:
    """Diff LLM1 vs LLM2 step4 label/bio results."""
    logger.info("=== Step4 Diff: LLM1 vs LLM2 labels ===")

    llm1_cache = load_json_cache(logs_dir / "llm1_step4_labels.json")
    llm2_cache = load_json_cache(logs_dir / "llm2_step4_labels.json")

    all_diffs: list[dict] = []

    for name, ds_s4 in llm1_cache.items():
        vf_s4 = llm2_cache.get(name)
        if not vf_s4:
            continue

        person_diffs: list[dict] = []
        person_diffs += diff_bio_fields(
            ds_s4.get("raw_bio", {}), vf_s4.get("raw_bio", {})
        )
        person_diffs += diff_label_fields(ds_s4, vf_s4)
        person_diffs += diff_corruption(ds_s4, vf_s4)
    ...
    output_path = logs_dir / "step4_diff_report.json"
    output_path.write_text(json.dumps(all_diffs, ensure_ascii=False, indent=2), encoding="utf-8")

    total = len(all_diffs)
    passes = sum(1 for d in all_diffs if d["verdict"] == "PASS")
    logger.info(f"Step4 Diff 完成: PASS={passes} / {total}")
